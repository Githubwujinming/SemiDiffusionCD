...
class CD(BaseModel):
    ...
    def get_current_visuals(self):
        ...
        if self.opt['phase'] == 'test':
            self.compare()
            out_dict['pred_cm'] = self.comp
        
        return out_dict
    def compare(self):
        pred_L_show = torch.argmax(self.pred_cm, dim=1, keepdim=True).detach().cpu()#(1,1,256,256)
        L = self.data['L'].unsqueeze(0).detach().cpu()
        tp = (L * pred_L_show)
        fp = (pred_L_show - tp)
        fn = L - tp
        ...
        comp[tp[:,:,:,0]==1] = torch.tensor([255.0,255.0,255.0]) / 255 #white
        comp[fp[:,:,:,0]==1] = torch.tensor([220.0,20.0,60.0]) / 255#red
        comp[fn[:,:,:,0]==1] = torch.tensor([0.0,0.0,205.0]) / 255# blue
    ...
    def save_network(self, epoch, is_best_model = False):
        ...
        if is_best_model:
            best_cd_gen_path = os.path.join(
                self.opt['path']['checkpoint'], 'E{}_best_cd_model_gen.pth'.format(epoch))
            best_cd_opt_path = os.path.join(
                self.opt['path']['checkpoint'], 'E{}_best_cd_model_opt.pth'.format(epoch))
