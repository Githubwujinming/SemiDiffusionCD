...
class UNet(nn.Module):
    def __init__(
        ...
        for ind in reversed(range(num_mults)):
            ...
            for _ in range(0, res_blocks+1):
                ups.append(ResnetBlocWithAttn(#总共20层
    ...
    def forward(self, x, time, feat_need=False):
        ...
        # Diffusion encoder
        feats = [x]#downs总共15层，因此feats是长度15的特征list
...
class UNetV2(nn.Module):
    ...
    def forward(self, x, time, feat_need=False):
        ...
        # Output encoder and decoder features if feat_need
        if feat_need:
            fe = [fe[i] for i in self.feat_scales]
            fd = Reverse(fd)
            fd = [fd[i] for i in self.feat_scales]
            return fe, fd# 返回中间特征
        else:
            return x# 返回最后的生成图片
