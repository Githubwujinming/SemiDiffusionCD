...
if __name__ == "__main__":
    ...
    parser.add_argument('-log_eval', action='store_true')# 这个只在ddpm_train用到，用于看扩散过程中的图片
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if 'train' in phase and args.phase != 'test':
    ...
    best_mF1 = 0.0
    start_epoch = change_detection.begin_epoch
    if opt['phase'] == 'train':
        for current_epoch in range(start_epoch, n_epoch):         
            ...
            if wandb_logger:
                    wandb_logger.log_metrics({
                        'training/lr': change_detection.optCD.param_groups[0]['lr'],
        ...
        for current_step, test_data in enumerate(test_loader):
            ...
            if img_mode == 'single':
                ...
                gt_cm   = Metrics.tensor2img(visuals['gt_cm'].unsqueeze(1).repeat(1, 3, 1, 1), out_type=np.uint8, min_max=(0, 1))  # uint8
                # pred_cm = Metrics.tensor2img(visuals['pred_cm'].unsqueeze(1).repeat(1, 3, 1, 1), out_type=np.uint8, min_max=(0, 1))  # uint8
                pred_cm = Metrics.tensor2img(visuals['pred_cm'], out_type=np.uint8, min_max=(0, 1))
                # Save imgs
                Metrics.save_img(
                    img_A, '{}/{}_A.png'.format(test_result_path, image_id))
                Metrics.save_img(
                    img_B, '{}/{}_B.png'.format(test_result_path, image_id))
                Metrics.save_img(
                    pred_cm, '{}/{}_comp.png'.format(test_result_path, image_id))
                Metrics.save_img(
                    gt_cm, '{}/{}_L.png'.format(test_result_path, image_id))
            else:
                # grid img
                visuals['pred_cm'] = visuals['pred_cm']*2.0-1.0
                visuals['gt_cm'] = visuals['gt_cm']*2.0-1.0
